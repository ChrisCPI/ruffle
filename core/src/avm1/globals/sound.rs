...
struct SoundData<'gc> {
    ...
    /// The target display object that will "own" this sound.
    ...
    target: Option<MovieClipReference<'gc>>,
    //target: Option<Value<'gc>>,
    /// Whether this sound should use global sound (owner is Null or Undefined).
    use_global_sound: bool,
...
impl<'gc> Sound<'gc> {
    ...
    pub fn owner(self, activation: &mut Activation<'_, 'gc>) -> Option<DisplayObject<'gc>> {
        /*if let Some(target) = self.0.target {
            ...
            activation
                .resolve_target_display_object(start_clip, target, false)
                .ok()?
            target.resolve_reference(activation).
        } else {
            None
        }*/

        /*let (_, _, clip) = self.0.target?.resolve_reference(activation)?;
        tracing::info!("owner() {:?} {:?}", clip, clip.name());
        Some(clip)*/

        // TODO: This logic is not entirely correct.
        // It will not work when providing a string path to a nested MC.
        let start_clip = activation.target_clip_or_root();
        //tracing::info!("buh {:?} {:?}", start_clip.path(), self.0.target?.resolve_reference(activation));
        let target = self.0.target?.coerce_to_string(activation);
        activation
...
fn constructor<'gc>(
    ...
    //let mut target = args.get(0).copied();

    /*if let Some(value) = target && let Value::Object(object) = value
        && let Some(mcr) = MovieClipReference::try_from_stage_object(activation, object)
    {
        MovieClipReference::try_from_stage_object(activation, object)
        target = Some(Value::MovieClip(mcr));
    }*/

    let use_global_sound = args
        .get(0)
        .is_none_or(|v| matches!(v, Value::Null | Value::Undefined));

    let target = if use_global_sound {
        None
    } else {
        match args {
            ...
            [value] => {
                let path = value.coerce_to_string(activation)?;
                MovieClipReference::new_from_path(activation, path)
            }
            _ => None,
        }
    };

    let sound = Sound::empty(activation.gc(), target, use_global_sound);
